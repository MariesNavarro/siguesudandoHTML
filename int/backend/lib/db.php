...
function validafechas(&$cad)
{
  ...
  $contador=0;
  $link=connect();
  $consulta = "select setting,value,NOW(),TIME_TO_SEC(TIMEDIFF(NOW(), value)) valor from gtrd_settings where Module='Validez';";
...
function cupon_php($idClient,$idprom,$ip)
{
  $count=0;
  $link=connect();
  ...
  if ($resultado = mysqli_query($link, $consulta)) {
    while ($fila = mysqli_fetch_row($resultado)) {
        $count++;
        promvalidestado($idClient,$ip,$idprom,$link);
     }
     if($count<1)
     {
...
function promvalidestado($idClient,$ip,$idprom,$link)
{
  $salida          = 0;
  $country_code    = '';
  $ip_address      = $ip;
  ...
  $query2="select * from gtrd_promociones_estados a inner join gtrd_estados b on a.id_estado=b.id where b.codigo_estado='".$estado."' and b.pais='".$country_code."' id_promo=".$idprom.";";
...
function getcupon($link,$client,$idClient,$promo)
{
  $count=0;
  ...
  if ($resultado = mysqli_query($link, $query1)) {
    while ($fila = mysqli_fetch_row($resultado)) {
        ...
        {
        	echo 'Ya has recibido un codigo,debes esperar un lapso de 24 horas para solicitar otro. Tiempo Transcurrido desde tu ultima solicitud '.$fila['TiempoTranscurrido'].' aun deves esperar '.$fila['TiempoRestante'].'<br/>';
        }
     }
     if($count<1)
     {
...
function getcodigo($link,$promo,$ip,$huella)
{
  ...
  if ($resultado = mysqli_query($link, $query2)) {
    while ($fila = mysqli_fetch_row($resultado)) {
      ...
      echo '<img src="img/promoMob'.$fila['codigo'].'.png"/><br/><br/><br/><br/><br/><img src="img/promo'.$fila['codigo'].'.png"/><a href="img/promoMob'.$fila['codigo'].'.png" download>Descargar</a>';
      update_codigos($datos['codigo'],$ip,$huella,$link);
    }
    if($count<1)
    {
